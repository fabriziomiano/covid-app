...
def plot_overview(data, territory=""):
    ...
    axs[0, 0].set_title('In intensive care {}'.format(territory.upper()))
    axs[0, 1].plot(dates, hospitalized_w_symptoms, 'tab:orange')
    axs[0, 1].set_title('Hospitalized with symptoms {}'.format(territory.upper()))
    axs[1, 0].plot(dates, total_cases)
    axs[1, 0].set_title('Total cases {}'.format(territory.upper()))
    axs[1, 1].plot(dates, healed, 'tab:green')
    axs[1, 1].set_title('Healed (dismissed) {}'.format(territory.upper()))
    for ax in axs.flat:
